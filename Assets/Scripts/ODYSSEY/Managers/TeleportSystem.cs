...
public class TeleportSystem : MonoBehaviour, ITeleportSystem, IRequiresContext
{
    ...
    {
        if (_c.Get<IFollowUserController>().IsFollowing)
        {
            Logging.Log("[TeleportSystem] Trying to teleport to user, while in Follow mode..");
            return;
        }
    ...
    public void OnTeleportToSpace(string spaceUid)
    {
        if (_c.Get<IFollowUserController>().IsFollowing)
        {
            Logging.Log("[TeleportSystem] Trying to teleport to space, while in Follow mode..");
            return;
        }
